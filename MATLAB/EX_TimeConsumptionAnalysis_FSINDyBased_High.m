...
figpath = './figures/';
isOutputToFile = 1;
baseFileName = 'TimeConsumptionAnalysisFSINDyBased_High';
isOverwrite = 1;

if ~exist("TimeConsumptionAnalysisFSINDyBased_High.mat") || isOverwrite
    ...
    save('TimeConsumptionAnalysisFSINDyBased_High.mat','model', 'record');
else
...
figure;
bar(1:length(model), T.MeanPerStep./(T_list.*Nf.*Dim), 'EdgeColor', 'none', 'FaceColor', '#6888F5');

hold on;
er = errorbar(T.MeanPerStep./(T_list.*Nf.*Dim), T.StdPerStep./(T_list.*Nf.*Dim), 'LineWidth', 1.5);
er.Color = [0 0 0];
er.LineStyle = 'none';
