...
for i = 1:n
    ...
    semilogy(y1, '-', 'LineWidth',1.5, 'Marker', '*', 'MarkerSize', 10);

    set(gca, 'YScale', 'log');
    ylim([1e-8 1e8])
    
    yticks(logspace(-8,8,3))
    
    yyaxis right;
    y2 = history.MRE{i}(:);
    
    semilogy(y2, '.-', 'LineWidth',1.5, 'Marker', 'o', 'MarkerSize', 10);

    set(gca, 'LineWidth',1.2, 'FontSize', 12, 'FontName', 'Times');
    ylabel(['X_',num2str(i)], 'FontSize',12);
    
    title(number_label{i}, 'Units', 'normalized', 'Position', [0.04, 0.65, 0], 'FontWeight', 'bold');
...
func_name = 'test_4D_Hyper_Rossler';
mg = ModelGenerate(Charset, Xi, func_name);

[t_span,x_true]=ode45(@(t,x) Hyper_Rossler(t,x,a,b,c,d),tspan,x0,options);   % true model
