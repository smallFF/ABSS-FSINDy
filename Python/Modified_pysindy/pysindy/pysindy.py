...
class SINDy(BaseEstimator):
    ...
    def fit(
        ...
        if self.feature_names is None:
            feature_names = []
            if self.n_features_in_ - self.n_control_features_ <= 10:
                for i in range(self.n_features_in_ - self.n_control_features_):
                    feature_names.append("x[%d]" % i)
            else:
                for i in range(self.n_features_in_ - self.n_control_features_):
                    feature_names.append("x[%-2d]" % i)
                    
            if self.n_control_features_ <= 10:
                for i in range(self.n_control_features_):
                    feature_names.append("u[%d]" % i)
            else:
                for i in range(self.n_control_features_):
                    feature_names.append("u[%-2d]" % i)
    ...
    def print(self, lhs=None, precision=3):
        ...
        codes_list = []
        codes_list.append('def rhs(t, x):\n')
        codes_list.append(' '*4+'return [\n')
        for i in range(0, len(eqns)):
            codes_list.append(' '*8+eqns[i]+',\n')
        codes_list.append(' '*4+']')
        
        self.eqns_str = "".join(codes_list)
        
        # exec(f"""{self.eqns_str}""")
        # print(rhs(0, np.random.random(15)))
        
        # file_contents = open('generated_model.py').read()
        # exec(file_contents, globals())
        # print(ode(0, np.random.random(15)))
        
        # f.close()
    # def generate(self, name='rhs'):
    #     with open('generated_model.py', 'w') as f:
    #         f.write('def '+name+'(t, x):')
    #         for i = 1:length(self.equations(precisi))
    ...
    def simulate(
        ...
        if self.discrete_time:
            ...
            # from generated_model import rhs
            
            # # defined in print
            # codes_str = self.eqns_str
            # exec(codes_str, globals(), globals())
            # exec(codes_str)
            # print(codes_str)
            # print(rhs)
            ...
            codes_str = self.eqns_str
            local_vars = {}
            exec(codes_str, globals(), local_vars)
            rhs = local_vars['rhs']
            # print(rhs)
